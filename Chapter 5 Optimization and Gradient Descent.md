...
Transformer も、損失を小さくするようにパラメータを更新して学習します。

実際には単純な SGD だけでなく Adam や AdamW のような最適化アルゴリズムがよく使われますが、根本の考え方は同じです。予測し、損失を計算し、逆伝播で勾配を求め、重みを少しずつ更新します。

**ミニ演習**

- 第5章の PyTorch コードで、学習率 `lr` を `0.01`、`0.1`、`1.0` に変えて、損失の下がり方を比べてみましょう。
