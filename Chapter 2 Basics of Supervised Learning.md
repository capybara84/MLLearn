...
大規模言語モデルの事前学習は、文章そのものから入力と正解のペアを作る点で、教師あり学習に近い構造を持っています。

たとえば「吾輩は猫である」という文から、「吾輩は」から「猫」を予測する、という学習データを作れます。Transformer はこのような大量のペアを使って、次のトークンを予測する力を身につけます。

**ミニ演習**

- 「今日はとても暑いです」という文から、次トークン予測用の入力と正解のペアを3つ作ってみましょう。
- メールのスパム判定、家の価格予測、画像分類を、分類問題と回帰問題に分けてみましょう。
- 第2章の PyTorch コードで、`x` や `y` の値を変えると損失がどう変わるか確認してみましょう。
